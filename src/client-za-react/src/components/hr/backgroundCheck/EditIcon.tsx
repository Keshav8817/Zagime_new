import { deleteBackgroundCheck } from "../../../pages/hr/backgroundCheck/service";
...
const EditIcon: FC<any> = ({
  ...
  const handleClose = () => {
    navigate(`../backgroundCheck/${id}`);
    setAnchorEl(null);
    setAddNew(false);
  };

  const handleDelete = () => {
    deleteBackgroundCheck(Number(childId)).then(() => {
      navigate(`../backgroundCheck/${id}`);
    });
    // .then(() => setOpenModel(false))
    // .then(() => readAllGoalsAndObjective(targetValue || null));
    // setAddNew(false);
  };
